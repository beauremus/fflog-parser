...
def fetch_fight_health_data(fight_id: int) -> dict | None:
    ...
    try:
        ...
        if "series" in report_graph_data:
            for series_item in report_graph_data["series"]:
                if (series_item.get("id") == BOSS_ACTOR_ID and
                    series_item.get("name") == EXPECTED_BOSS_NAME and
                    series_item.get("type") == "Boss"):
        ...
        if boss_health_series:
            ...
            print(f"Successfully extracted data for boss (ID: {BOSS_ACTOR_ID}) in fight {fight_id}.")
            return {
                "fight_id": fight_id,
                "health_percentage_series": health_percentage_series, # [timestamp, health_percent]
            }
        else:
            print(f"Warning: Boss series (ID: {BOSS_ACTOR_ID}, Name: '{EXPECTED_BOSS_NAME}') not found for fight {fight_id}.")
            return None
...
def main():
    ...
    if all_fights_boss_data:
        ...
        if 1 in all_fights_boss_data:
            ...
            for timestamp, hp_percent in all_fights_boss_data[1]["health_percentage_series"]:
